...
// Função para verificar se o conteúdo entre as chaves é uma variável válida
bool is_valid_variable(const char *str, int len) {
    for (int i = 0; i < len; i++) {
        if (!(isalnum(str[i]) || str[i] == '_')) {  // Verifica se é alfanumérico ou _
            return false;  // Se não for, a variável é inválida
        }
    }
    return true;  // Válida se não encontrou nenhum caractere inválido
}
...
void process_string(const char *str) {
    ...
    while (*cursor != '\0') {
        ...
        if (*cursor == '}') {
            ...
            if (is_valid_variable(start_braces, valor_len)) {
                ...
                // Se a variável não é válida, trata o conteúdo como parte do texto
                printf("Texto antes das chaves: \"%.*s\"\n", valor_len + 2, start_braces - 1);
            }
