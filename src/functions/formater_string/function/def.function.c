...
char *Candango_function_text(Candango_args_render *self, const char *chunk, size_t index, long long size_chunk, LuaCEmbed *machine){
  ...
  const char *finded_end_key = strstr(text_working, CANDANGO_KEY_END_FUNCTION);
  ...
  const char *init_block_last_end_key = finded_end_key + strlen(CANDANGO_KEY_END_FUNCTION);
